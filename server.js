  ...
  try {
    ...
    if (search && search.trim().length >= 2) {
      ...
      videos = videos.filter(v => {
        ...
        const tagMatch = v.tags && v.tags.some(tag => subsequenceMatch(tag, searchTerm));
        const exactMatch = v.title.toLowerCase().includes(searchTerm.toLowerCase());
        const wordsMatch = searchTerm.toLowerCase().split(' ')
          .every(word => word.length >= 2 && v.title.toLowerCase().includes(word));
...
app.get('/api/videos/:id/related', async (req, res) => {
  try {
    ...
    const available = videos.filter(v => v.id !== req.params.id && v.published !== false);
    ...
    const currentWords = currentTitle.split(/[\s\-_.,!@#$%^&*()]+/).filter(w => w.length >= 2);
    const titleMatches = available.filter(v => {
      const vt = v.title.toLowerCase();
      const vw = vt.split(/[\s\-_.,!@#$%^&*()]+/).filter(w => w.length >= 2);
      const hasShared = currentWords.some(cw => vw.some(ww => {
      ...
      const titleSub = subsequenceMatch(vt, currentTitle.substring(0, Math.min(10, currentTitle.length)));
      return hasShared || titleSub;
    });

    const scored = titleMatches.map(v => {
      const vt = v.title.toLowerCase();
      const vw = vt.split(/[\s\-_.,!@#$%^&*()]+/).filter(w => w.length >= 2);
      let score = 0;
      currentWords.forEach(cw => {
        vw.forEach(ww => {
          ...
          else if (subsequenceMatch(cw, ww) || subsequenceMatch(ww, cw)) score += 3;
        });
      });
      return { ...v, score };
    ...
    if (related.length < 4) {
      const more = available.filter(v => v.category === currentVideo.category && !related.find(r => r.id === v.id));
      related = [...related, ...more.slice(0, 4 - related.length)];
    }

    // Method 3: same tags
    if (related.length < 4 && currentVideo.tags?.length) {
      const more = available.filter(v => v.tags && v.tags.some(t => currentVideo.tags.includes(t)) && !related.find(r => r.id === v.id));
      related = [...related, ...more.slice(0, 4 - related.length)];
    }

    // Method 4: random fill
    if (related.length < 4) {
      const remain = available.filter(v => !related.find(r => r.id === v.id));
...
app.get('/api/admin/videos', requireAuth, async (req, res) => {
  try {
    ...
    switch (sort) {
      case 'oldest': videos.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)); break;
      case 'views': videos.sort((a, b) => (b.views || 0) - (a.views || 0)); break;
      default: videos.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    }
...
app.put('/api/admin/videos/:id', requireAuth, upload.single('thumbnail'), async (req, res) => {
  try {
    ...
    const { title, embedUrls, thumbnailUrl, duration, category, tags, notes, downloadLink, published } = req.body;

    videos[idx] = {
      ...videos[idx],
      title,
      embedUrls: JSON.parse(embedUrls || '[]'),
      thumbnail: req.file ? `/uploads/${req.file.filename}` : thumbnailUrl || videos[idx].thumbnail,
      duration,
      category: category || 'none',
      tags: JSON.parse(tags || '[]'),
      notes: notes || '',
...
app.delete('/api/admin/videos/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (videos.length === filtered.length) return res.status(404).json({ error: 'Video not found' });
